    ...
    <Container>
      ...
      <LoginScreenBox>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <Input leftIcon={<MdPassword />} label={'confirm password'} type={'password'} placeHolder={'********'} errorMessage={errors.passwordConfirmation?.message} register={register} registerName={'passwordConfirmation'}/>
          <Center> Already have an account? <Link to={'/'}>Sign in</Link> </Center>
          {loading ? <Button variant={'userauth'} isValid={!loading} title={<Spinner style={{fontSize: '32px'}}/>}/>  : <Button variant={'userauth'} isValid={isValid} title={'Sign up'} type="submit"/> }
        </form>
      </LoginScreenBox>
    </Container>
  )
